#!/usr/bin/env python3
"""Import denver.xlsx sheet Felix into Felix's Firestore profile."""
...
XLSX_PATH = Path("/Users/ganyifei/Desktop/非常重要的东西/vozol-shop-tracker/denver.xlsx")
SHEET_NAME = "Felix"
TOKEN_PATH = "/Users/ganyifei/.config/configstore/firebase-tools.json"
PROJECT = "vozol-orlando-management"
OWNER_UID = "8AZjHby0Y0XHwpmtGz1WGoNOeL22"
TEAM_ID = "denver"
CHAIN_MIN_STORES = 5
MAX_TRAFFIC_NOTES = 2
...
def looks_like_address(text):
    ...
    if not value or not re.search(r"\d", value):
        return False
    if re.search(r"\bCO\b", value, re.I) or re.search(r"\d{5}\b", value):
        return True
    return bool(re.search(r"\b(rd|st|ave|blvd|dr|hwy|pkwy|way|ln|ct)\b", value, re.I))
...
def normalize_raw_row(raw):
    """Return dict of logical fields, repairing known sheet misalignments."""
    cols = list(raw) + [None] * max(0, 18 - len(raw))
    person = blank(cols[0])

    # Thornton block: city leaked into person, columns shifted left by 1,
    # with an extra freeform owner description in former address slot.
    if known_city(person) and looks_like_address(cols[2]):
        city = known_city(person)
        name = blank(cols[1])
        address = blank(cols[2])
        owner_extra = blank(cols[3])
        shop_type = blank(cols[4]) or "smoke shop"
        notes = join_notes(cols[5], cols[6])
        store_count = cols[7]
        visit_raw = cols[8]
        owner_name = blank(cols[9]) or owner_extra
        phone = cols[10]
        tier = cols[11]
        distributor = cols[12]
        sell_status = cols[13]
        ...
        coop = cols[16]
        test_case = cols[17]
        # Address city may differ (e.g. Northglenn)
        city = normalize_city(city, address)
        return {
            "name": name,
            "address": address,
            "city": city,
    ...
    # Denver tail rows: city in name col, shop name in address col, real address in notes col.
    if (
        person.lower() == "felix"
        and known_city(cols[2])
        and not looks_like_address(cols[3])
        and looks_like_address(cols[5])
    ):
        city = known_city(cols[2])
        name = blank(cols[3])
        address = blank(cols[5])
        shop_type = blank(cols[4]) or "smoke shop"
        notes = blank(cols[6])
        return {
            "name": name,
    ...
    if person.lower() != "felix":
        return None

    name = blank(cols[2])
    address = blank(cols[3])
    if not name and not address:
        return None
    if not name and address:
        # Keep address-only rows with a usable placeholder name.
    ...
    return {
        "name": name,
        "address": address,
        "city": normalize_city(cols[1], address),
        "shop_type": blank(cols[4]) or "smoke shop",
        "notes": join_notes(cols[5], cols[6]),
        "store_count": cols[7],
        "visit_raw": cols[8],
...
def load_rows():
    ...
    for raw in ws.iter_rows(min_row=2, values_only=True):
        ...
        shops.append({
            ...
            "city": parsed.get("city") or "Denver",
...
def main():
    ...
    if existing_docs:
        raise SystemExit("Felix already has shops; aborting to avoid duplicates.")
