#!/usr/bin/env python3
"""Import denver.xlsx sheet BO into Bo's Firestore profile."""
...
OWNER_UID = "YPYwFso1CERay13Y9vTx2hmwarP2"
TEAM_ID = "denver"
CHAIN_MIN_STORES = 5
MAX_TRAFFIC_NOTES = 2
...
def parse_all_dates(text):
    dates = []
    raw = blank(text)
    for match in re.finditer(r"(\d{1,2})[./](\d{1,2})(?:[./](\d{2,4}))?", raw):
...
def fragment_date(text, fallback):
    dates = parse_all_dates(text)
    if dates and re.match(r"^[\*\-\s]*\d{1,2}[./]\d{1,2}", blank(text)):
        return dates[0]
    return fallback
...
def map_status(sell_status):
    ...
    if "拒绝" in value:
        return "no_interest"
    if value in ("待跟进", "寄售") or not value:
        return "follow_up"
    return "follow_up"
...
def load_rows():
    ...
    for raw in ws.iter_rows(min_row=2, values_only=True):
        if not raw or len(raw) < 3:
            continue
        name = blank(raw[2] if len(raw) > 2 else "")
        if not name:
            continue
        visit_raw = blank(raw[15] if len(raw) > 15 else "")
        visit_date = parse_visit_date(visit_raw)
        info = blank(raw[16] if len(raw) > 16 else "")
        popup = blank(raw[11] if len(raw) > 11 else "")
        note_source = info or popup
        address = blank(raw[3] if len(raw) > 3 else "")
        city = normalize_city(raw[1] if len(raw) > 1 else "", address)
        shops.append({
            "name": name,
            "address": address,
            "city": city,
            "shop_type": blank(raw[4] if len(raw) > 4 else "") or "smoke shop",
            ...
            "sell_status": blank(raw[10] if len(raw) > 10 else ""),
            "coop": blank(raw[13] if len(raw) > 13 else ""),
            "info": note_source,
            "sample": is_yes(raw[12] if len(raw) > 12 else ""),
            "test_case": is_yes(raw[14] if len(raw) > 14 else ""),
...
def main():
    ...
    if existing_docs:
        raise SystemExit("Bo already has shops; aborting to avoid duplicates.")
