#!/usr/bin/env python3
"""Set 周依凡 & Vindy to dallas, then import 达拉斯店铺拜访.xlsx."""
...
XLSX_PATH = Path("/Users/ganyifei/Desktop/非常重要的东西/vozol-shop-tracker/达拉斯店铺拜访.xlsx")
SHEET_NAME = "Sheet1"
...
OWNERS = [
    {
        "label": "周依凡",
        "uid": "YAUnt6mVRpf2dT1JlGkziroUiKG2",
        "names": {"周依凡", "yifan", "yi fan"},
    },
    {
        "label": "Vindy Fu",
        "uid": "iFnC9sDfUWY45ApM7gp5IQIGMgZ2",
        "names": {"vindy", "vindy fu"},
    },
]

CITIES = [
    "Farmers Branch", "Grand Prairie", "Carrollton", "Richardson", "Lewisville",
    "Arlington", "Fort Worth", "McKinney", "Garland", "Irving", "Frisco",
    "Denton", "Plano", "Mesquite", "Allen", "Addison", "Dallas", "Houston",
]
...
def person_matches(person, owner):
    key = blank(person).lower()
    return any(key == n or n in key for n in owner["names"])
...
def load_rows():
    ...
    for raw in ws.iter_rows(min_row=2, values_only=True):
        ...
        if not name:
            continue
        person = blank(raw[0])
        visit_raw = blank(raw[16] if len(raw) > 16 else "")
        visit_date = parse_visit_date(visit_raw)
        info = blank(raw[17] if len(raw) > 17 else "")
        popup = blank(raw[12] if len(raw) > 12 else "")
        note_source = "\n".join([x for x in (info, popup) if x])
        address = blank(raw[3])
        city = normalize_city(raw[6] if len(raw) > 6 else "", address)
        if not city or city == "Dallas":
        ...
        rows.append({
            ...
            "tier": parse_tier(raw[9] if len(raw) > 9 else ""),
            "owner_name": blank(raw[7] if len(raw) > 7 else ""),
            "phone": blank(raw[8] if len(raw) > 8 else ""),
            "sell_status": blank(raw[11] if len(raw) > 11 else ""),
            "coop": map_willingness(raw[14] if len(raw) > 14 else ""),
            "info": note_source,
            "sample": is_yes(raw[13] if len(raw) > 13 else ""),
            "test_case": is_yes(raw[15] if len(raw) > 15 else ""),
            "distributor": blank(raw[10] if len(raw) > 10 else ""),
...
def patch_profile_dallas(token, uid):
    query = "&".join(
        f"updateMask.fieldPaths={urllib.parse.quote(path)}"
        for path in ("team_id", "area_id")
    )
    request(
        "PATCH",
        f"{BASE}/profiles/{uid}?{query}",
        token,
        {"fields": {"team_id": sval(TEAM_ID), "area_id": sval(AREA_ID)}},
    )
...
def import_owner(token, owner, all_rows):
    uid = owner["uid"]
    ...
    print(f"\n=== {owner['label']} / {name} ({uid}) old_team={team} ===")

    patch_profile_dallas(token, uid)
    print("  profile -> dallas")
    ...
    rows = [row for row in all_rows if person_matches(row["person"], owner)]
    ...
    print(f"  rows={len(rows)} merged={len(shops)}")
    print(
        f"  write {len(shops)} shops "
...
def main():
    ...
    token = load_token()
    all_rows = load_rows()
    print(f"Loaded {len(all_rows)} rows from {XLSX_PATH.name}")
    for owner in OWNERS:
        import_owner(token, owner, all_rows)
    print("\nDone.")
