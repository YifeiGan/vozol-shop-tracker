#!/usr/bin/env python3
"""Import great_lakes.xlsx sheet Celia into HAIZHEN SUN's Firestore profile."""
...
OWNER_UID = "FiEV9Lk8KTXiu6FSfjzS6FKkubF2"
TEAM_ID = "great_lakes"
CHAIN_MIN_STORES = 5
MAX_TRAFFIC_NOTES = 2
...
def normalize_city(address):
    ...
    if match:
        ...
        if city and not re.search(r"\d", city) and 2 < len(city) < 40:
            return city
    # e.g. "3914 s Harlem Lyons iL 65034" or "... Naperville"
    loose = re.search(r"\b([A-Za-z][A-Za-z .']+?)\s+(IL|IN|MI|OH|WI|MN)\b", addr, re.I)
    if loose:
        city = blank(loose.group(1))
        # drop trailing street tokens if any slipped in
        parts = city.split()
        if parts and not re.search(r"\d", city) and 2 < len(city) < 40:
            if len(parts) > 3:
                city = " ".join(parts[-2:])
...
def main():
    ...
    page_token = None
    while True:
        url = f"{BASE}/profiles/{OWNER_UID}/shops?pageSize=300"
        if page_token:
            url += f"&pageToken={urllib.parse.quote(page_token)}"
        page = request("GET", url, token)
        existing_docs.extend(page.get("documents") or [])
        page_token = page.get("nextPageToken")
        if not page_token:
            break
    print(f"Existing shops before import: {len(existing_docs)}")
    existing_addrs = {
        addr_key((doc.get("fields") or {}).get("address", {}).get("stringValue", ""))
        for doc in existing_docs
    }
    existing_addrs.discard("")
    ...
    shops = OrderedDict(
        ...
        if not addr_key(v.get("address") or "") or addr_key(v.get("address") or "") not in existing_addrs
    )
    print(f"Skip existing addresses: {before - len(shops)}; will import: {len(shops)}")
    writes = []
